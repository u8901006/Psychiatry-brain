#!/usr/bin/env python3
"""Generate index.html listing all psychiatry daily reports."""

import glob
import os
from datetime import datetime

html_files = sorted(glob.glob("docs/psychiatry-*.html"), reverse=True)
links = ""
for f in html_files[:30]:
    name = os.path.basename(f)
    date = name.replace("psychiatry-", "").replace(".html", "")
    try:
...
<title>Psychiatry Brain · 精神醫學文獻日報</title>
<style>
...
<div class="container">
  <div class="logo">🧠</div>
  <h1>Psychiatry Brain</h1>
  <p class="subtitle">精神醫學文獻日報 · 每日自動更新</p>
  <p class="count">共 {total} 期日報</p>
  <ul>{links}</ul>
  <footer>
    <p>Powered by PubMed + NVIDIA AI · <a href="https://github.com/u8901006/Psychiatry-brain">GitHub</a></p>
  </footer>
</div>
</body>
</html>"""
